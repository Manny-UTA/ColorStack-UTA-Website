...
import { IMAGE_SLOTS, FLYER_SLOTS, OFFICER_PHOTO_SLOTS, OFFICER_PASSCODE, MAX_IMAGE_BYTES } from "@/lib/content";
          ...
          <form onSubmit={handleAdminLogin} className="max-w-sm">
            ...
            <div className="flex gap-3">
              <input
                ...
                aria-label="Officer passcode"
                className={`flex-1 min-w-0 bg-white border rounded-sm whitespace-nowrap px-4 py-3 text-sm outline-none ${passcodeError ? "border-red-400" : "border-navy/20 focus:border-brass"}`}
              />
              <button type="submit" className="bg-navy text-cream font-bold uppercase tracking-wide text-xs px-5 py-3 rounded-sm whitespace-nowrap shrink-0">Unlock</button>
            </div>
