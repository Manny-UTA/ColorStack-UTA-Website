  ...
  return (
    <section className="cs-sponsors" aria-labelledby="sponsor-banner-title">
      <div className="cs-shell cs-sponsor-heading">
        <h2 id="sponsor-banner-title" className="cs-eyebrow">Our sponsors & partners</h2>
      ...
      <div className="cs-sponsor-window" data-paused={paused}>
        <div className="cs-sponsor-track">
          <ul className="cs-sponsor-group" aria-label="Sponsors and partners">{PARTNERS.map(name => <li key={name}>{name}</li>)}</ul>
          <ul className="cs-sponsor-group cs-sponsor-copy" aria-hidden="true">{PARTNERS.map(name => <li key={name}>{name}</li>)}</ul>
        </div>
      </div>
    </section>
  );
